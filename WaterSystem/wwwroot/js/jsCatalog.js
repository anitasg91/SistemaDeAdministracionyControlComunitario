function showModalbyCatalog() {
    ...
    switch (IdCatalog) {
        ...
        //    break;
        case "1":
            $("#lblTitleWaterMeterNE").html("<i class='fas fa-clock fa-lg'></i> Nueva Lectura");
...
function createTableGral(lista) {
    ...
    switch (IDCatalog) {
        case 1:
            theadTitle = ["#", "Descripción", "Inicial", "Limite", "Monto", "<i class='fas fa-cog fa-1x'></i>"];
            newLista = lista.tipoLectura;
            break;
        case 2:
            theadTitle = ["#", "Manzana", "Consecutivo", "Usuario Alta", "<i class='fas fa-cog fa-1x'></i>"];
            newLista = lista.folio;
            break;
        default:
     theadTitle = ["#", "Descripción", "<i class='fas fa-cog fa-1x'></i>"];
            break;
    }
    createTableCatalog("table_id", theadTitle, newLista, "#dvTablaCatalog", IDCatalog);
}
...
function validateEmptyFieldsWaterMeter() {
    ...
    if (lAct != "" && Number(lAnt) > Number(lAct)) {
        $(".LecturaAnterior").after("<label class='validar'>No puede ser mayor que la cantidad limite.</label>");
        error++;
    }
    //if (lAct == "") {
