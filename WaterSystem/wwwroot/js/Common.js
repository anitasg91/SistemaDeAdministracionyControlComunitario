...
function createSubmitBtn(id, dvClass, iclass, txt) {
    //return '<div id="' + id + '" class="' + dvClass + '" onclick="' + onclicFunction + '">'
    //    + '<i class="' + iclass + '" style = "color:white" ></i> ' + txt + '</div>';

    return '<button id="' + id + '" type="submit" class="' + dvClass + '"><i class="' + iclass + '" style="color:white"></i> ' + txt + '</button>'
...
function createFila(theadTitle, EsEncabezado, IdTipoTable) {
    ...
    $.each(theadTitle, function (index, value) {
        if (EsEncabezado) {
            ...
            switch (IdTipoTable) {
                case 1:
                    tdEnc += '<tr><td>' + value.concepto + '</td><td>' + value.metrosUsados + '</td><td>' + value.totalPagar + '</td></tr>';
                    break;
                //case 1:
                ...
                case 7:
                    if (value.id != "Ningún dato disponible en esta tabla" && value.id != "No data available in table") {
                        ...
                        var actionsOp = '<td style="with:9% !important;" class="dt-center">'
                            ...
                            + '</td>';
                        tdEnc +=
                            '<tr><td class="dt-center">'+ value.numero +'</td>'
...
function createFilaCatalog(theadTitle, EsEncabezado, IdTipoTable) {
    ...
    $.each(theadTitle, function (index, value) {
        if (EsEncabezado) {
            ...
            switch (IdTipoTable) {
                case 1:
                    tdEnc += '<tr><td class="dt-center ajustes">' + value.id + '</td><td>' + value.descripcion + '</td><td> ' + value.inicial
                        + '</td><td>' + value.limite + '</td><td>' + value.precio + '</td>' + actions + '</tr>';
                    break;
                case 2:
                    tdEnc += '<tr><td class="dt-center ajustes">' + value.id + '</td><td> Manzana ' + value.idManzana + '</td><td>' + value.consecutivo + '</td><td>' + value.usuarioAlta + '</td>' + actions + '</tr>';
                    break;
                case 3:
